...
| Rooms & Matchmaking | `server/matchmaker/`    | In Progress | Room lifecycle, lobby, matchmaking |
| Turn Engine | `core/turnengine/`      | Stubbed     | Turn progression, game state management |
| Persistence | `server/Database`       | In Progress | SQLite database with user login, game info tables; server session handling with login, registration, chat, game list requests |
...
| UI Components | `client/components/`    | In Progress | Reusable UI components |
| Game Rendering | `client/rendering/`     | In Progress | Board rendering for sample games |
| Move Validation | `rules/validation/`     | In progress | Server-side move validation pipeline |
| Leaderboard | `rules/leaderboard/`    | In Progress | Leaderboard scoring, match records, per-game leaderboard database |
| Tic-Tac-Toe | `games/tictactoe/`      | Complete    | Board initialization, move placement with validation, win condition checking, database serialization |
| Connect Four | `games/connectfour/`    | Complete    | Sample game implementation |
...
| Integration Tests | `test/.../integration/` | In Progress | End-to-end integration tests |
